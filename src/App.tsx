...
function ArmyBuilderApp() {
  ...
  const { list, index, summary, validation, remoteRevision } = useListStore();
  ...
  const createList = () => { resetForRace(user?.defaultRace ?? 'ZERG'); setSeedVisible(false); setPage('builder'); };
  const onImportFile = async (file: File) => { const result = importListFromJson(await file.text()); if (result.list) { setList(result.list); setSeedVisible(false); setPage('builder'); setToast('Lista importada.'); } else setToast(result.error ?? 'No se pudo importar el fichero.'); };
  const saveList = async () => { try { const saved = await saveRemoteList(list, remoteRevision); setRemoteRevision(saved.revision); setToast('Lista guardada en tu cuenta.'); } catch (error) { setToast(error instanceof Error ? error.message : 'No se pudo guardar la lista.'); } };
  const loadList = (loaded: typeof list, revision: number) => { setList(loaded); setRemoteRevision(revision); setSeedVisible(false); setPage('builder'); setToast('Lista cargada.'); };
  ...
  return <div className="app" data-race={list.race}>
    ...
    {page === 'builder' && <>
      ...
      <main className="content">{step === 'cards' && <StepCommandCards />}{step === 'units' && <StepMusterUnits />}{step === 'scenario' && <StepScenario />}{step === 'review' && <StepReview />}</main>
    </>}
