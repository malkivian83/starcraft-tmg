...
const KEYWORD_RULES: KeywordRule[] = [
  ...
  { pattern: /^CONCENTRATED FIRE(?:\s*\(\d+\))?/, text: { es: 'Este arma no puede retirar más de X modelos como bajas en un ataque.', en: 'This weapon cannot remove more than X models as casualties in an attack.' } },
  ...
  { pattern: /^DISPLACEMENT/, text: { es: 'El modelo líder puede terminar su movimiento solapando esta ficha o unidad y después se coloca en contacto peana con peana.', en: 'The Leading Model may end its move overlapping this Token or Unit, then is set in base-to-base contact.' } },
  ...
  { pattern: /^HEAL(?:\s*\(\d+\)|\s*\(X\))?/, text: { es: 'Retira X puntos de daño acumulado; no devuelve modelos destruidos.', en: 'Remove X points of accumulated Damage; destroyed models cannot return.' } },
  ...
  { pattern: /^IMPACT(?:\s*\(\d+\)(?:\s+\d+\+)?)?/, text: { es: 'Después de una Carga exitosa, cada modelo elegible genera X dados de Impacto; cada resultado Y+ coloca un dado en la Reserva de Armadura.', en: 'After a successful Charge, each eligible model generates X Impact dice; each result of Y+ places a die in the Armour Pool.' } },
  ...
  { pattern: /^MORPH(?:\s*\([^)]*\))?(?:\s+\d+)?/, text: { es: 'Transforma modelos de la unidad activa en la unidad indicada, respetando el suministro disponible.', en: 'Transforms models from the active Unit into the named Unit, subject to Available Supply.' } },
  { pattern: /^NON-LETHAL DAMAGE(?:\s*\(\d+\))?/, text: { es: 'Añade X puntos al marcador de daño sin retirar modelos; se combinan con el daño normal posterior.', en: 'Add X points to the Damage Marker without removing models; it combines with later standard Damage.' } },
  ...
  { pattern: /^PLACE(?:\s*\(\d+\))?/, text: { es: 'Retira el modelo líder y colócalo completamente dentro de X pulgadas; después coloca el resto manteniendo coherencia.', en: 'Remove and set the Leading Model Wholly Within X inches, then set the rest in Coherency.' } },
  ...
  { pattern: /^RESPAWN(?:\s*\(\d+\))?/, text: { es: 'Devuelve hasta X modelos destruidos a la unidad sin aumentar su actual tramo de suministro.', en: 'Return up to X destroyed models without increasing the Unit’s current Supply bracket.' } },
  { pattern: /^SHIELDED/, text: { es: 'El Escudo se suma a las Heridas del primer modelo y se pierde cuando el daño total supera ese valor.', en: 'The Shield adds to the first model’s Hit Points and is lost when total Damage exceeds it.' } },
  { pattern: /^SIDEARM/, text: { es: 'Permite usar esta arma además del arma normal del modelo; sus ataques se resuelven en un lote separado.', en: 'Allows the equipped model to use this weapon in addition to its normal weapon; resolve it as a separate Batch.' } },
  ...
  { pattern: /^SPECIALIST/, text: { es: 'Solo un modelo de la unidad puede estar equipado con esta arma.', en: 'Only one model in the Unit may be equipped with this weapon.' } },
  { pattern: /^SPILLOVER/, text: { es: 'Son impactos de un arma de plantilla que afectan a modelos fuera de la unidad objetivo principal.', en: 'These are hits from a Template Weapon affecting models outside the Primary target Unit.' } },
  { pattern: /^STATIONARY/, text: { es: 'La unidad empieza la ronda con este estado y lo pierde si cualquier modelo se mueve, es movido o es colocado.', en: 'The Unit starts the Round with this Status and loses it if any model moves, is moved, or is PLACED.' } },
