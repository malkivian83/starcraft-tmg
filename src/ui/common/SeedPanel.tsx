...
export function SeedPanel({ onImported }: { onImported: (list: ArmyList) => void }) {
  ...
  const importSeed = () => { for (const race of ['ZERG', 'TERRAN', 'PROTOSS'] as Race[]) { try { const result = decodeSeed(input, buildCatalogIndex(loadCatalog(race).catalog)); if (!result.list || result.status === 'corrupt' || result.list.race !== race) continue; onImported(result.list); setMessage(result.status === 'partial' ? t('importedPartial', { items: result.missing.join(', ') }) : result.status === 'version_mismatch' ? t('importedVersion') : t('importedOk')); return; } catch { /* Se prueba la siguiente raza. */ } } setMessage(t('invalidSeed')); };
